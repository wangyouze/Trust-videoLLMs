    ...
    def chat(self, messages: List[Dict[str, Any]], **generation_kwargs) -> Response:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 新增健壮性检查
                if not result or not hasattr(result, "choices") or not result.choices or not hasattr(result.choices[0], "message") or not hasattr(result.choices[0].message, "content"):
                    response = "OpenAI API returned empty or invalid result."
                    scores = None
                else:
                ...
                print("base64Frames:", len(base64Frames))
