...
class InternVL2_5(BaseChat):

    model_family = {
        "InternVL2_5-78B":"OpenGVLab/InternVL2_5-78B",
    ...
    def __init__(self, model_id: str, device: str = "cuda"):
        ...
        model_path = self.model_family[self.model_id]
    ...
    def chat(self, messages: List, **generation_kwargs):
        ...
        pixel_values, num_patches_list = load_video(video_path, num_segments=2, max_num=max_frames_num)
        pixel_values = pixel_values.to(torch.bfloat16).to(self.model.device)
        video_prefix = ''.join([f'Frame{i+1}: <image>\n' for i in range(len(num_patches_list))])
        question = video_prefix + user_message
        # Frame1: <image>\nFrame2: <image>\n...\nFrame8: <image>\n{question}
