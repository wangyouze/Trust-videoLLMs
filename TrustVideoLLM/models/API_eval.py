...
@registry.register_evaluator()
class SeepSeekAPIEvaluator(BaseEvaluator):
    evaluator_ids: List[str] = ['deepseek_api_eval']
    def __init__(self, evaluator_id: str, metrics_cfg: Dict[str, Any]) -> None:
        super().__init__(evaluator_id, metrics_cfg)
    ...
    def processor(self, preds: Sequence[Any], labels: Sequence[Any], extras: Sequence[Any]) -> Tuple[List[float], float]:
        assert len(preds) == len(labels)
