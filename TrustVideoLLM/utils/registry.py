class Registry:
    ...
    def register_videoLLM(cls):
        def wrap(model_cls):
            ...
            for model_id in model_cls.model_family:
                if model_id in cls.mapping["videoLLM_name_mapping"]:
                    raise KeyError(
                        "Name '{}' already registered for {}.".format(
                        model_id, cls.mapping["videoLLM_name_mapping"][model_id]
                        )
                    )
                cls.mapping["videoLLM_name_mapping"][model_id] = model_cls
            return model_cls
    ...
    def get_chatmodel_class(cls, name):
        return cls.mapping["videoLLM_name_mapping"].get(name, None)

    @classmethod
    def list_chatmodels(cls):
        return sorted(cls.mapping["videoLLM_name_mapping"].keys())
    ...
    def register_task(cls):
        ...
        def wrap(task_cls):
            ...
            from TrustVideoLLM.tasks import BaseTask
    ...
    def register_metrics(cls):
        ...
        def wrap(metrics_cls):
            from TrustVideoLLM.metrics.base import BaseDatasetMetrics, BasePerSampleMetrics
    ...
    def register_process(cls):
        ...
        def wrap(process_cls):
            from TrustVideoLLM.processes.base import BaseProcess
